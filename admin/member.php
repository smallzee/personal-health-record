<?php
  ...
  if(isset($_POST['ok-delete'])){
    $user_id = $_POST['ok-delete'];
    $del = $db->query("DELETE FROM scores WHERE user_id = '$user_id'");
    $del2 = $db->query("DELETE FROM users WHERE id = '$user_id'");

    set_flash("User deleted successfully","info");
    header("location:member.php?act=del");
    exit();
  }
